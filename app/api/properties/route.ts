...
import { addProperty, listProperties } from "@/lib/property-store";
...
function normalizePayload(input: unknown): PropertyInput {
  ...
  if (!title || !city || !address || !type || !description || images.length === 0) {
    throw new Error("Required fields are missing. Please include title, city, address, type, description and at least one image.");
  }
